...
export default function AdminPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Use new environment variable for Yuva admin password
    const adminPassword = process.env.NEXT_PUBLIC_YUVA_PASSWORD;
    const now = Date.now();
  ...
  const fetchData = () => {
    // Update reference from "jerseys" to "yuvaKurta"
    const kurtaRef = ref(database, 'yuvaKurta');
    onValue(kurtaRef, (snapshot) => {
      if (snapshot.exists()) {
            ...
            <TableBody>
              {filteredRegistrations.map((registration, index) => (
                ...
                >
                  ...
                  {/* Removed table cell for Jersey Text */}
                  {/* Removed table cell for Number */}
                  <TableCell>
                    {editMode[registration.key] ? (
                      <Input
